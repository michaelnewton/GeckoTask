...
export interface ValidationResult {
  rule: string;           // Rule identifier (e.g., "1.1", "2.1")
 ...
 * @returns Array of validation results
 */
export function validateTaskDueDate(due: string | undefined, title?: string): ValidationResult[] {
  const results: ValidationResult[] = [];
 ...
 * @returns Array of validation results
 */
export function validateTaskScheduled(scheduled: string | undefined): ValidationResult[] {
  const results: ValidationResult[] = [];
 ...
 * @returns Array of validation results
 */
export function validateTaskTags(tags: string[], waitingForTag: string): ValidationResult[] {
  const results: ValidationResult[] = [];
  ...
  const hasWaitingFor = tags.some(tag => 
    tag.toLowerCase() === waitingForTag.toLowerCase() ||
 ...
 */
function getSuggestedVerb(title: string): string {
