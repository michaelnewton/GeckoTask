...
exports.analyzeAllTasks = analyzeAllTasks;
exports.calculateHealthMetrics = calculateHealthMetrics;
exports.identifyStaleTasks = identifyStaleTasks;
exports.identifyQuickWins = identifyQuickWins;
exports.identifyMustMoveItems = identifyMustMoveItems;
exports.identifyOversizedTasks = identifyOversizedTasks;
exports.identifyTasksNeedingBreakdown = identifyTasksNeedingBreakdown;
exports.analyzeRecurringTasks = analyzeRecurringTasks;
exports.generateCleanupSuggestions = generateCleanupSuggestions;
const obsidian_1 = require("obsidian");
const TaskTrackingService_1 = require("./TaskTrackingService");
 ...
 */
async function analyzeAllTasks(app, settings, plugin) {
    // Load all tasks
    const tasks = await loadAllTasks(app, settings);
    // Load tracking data
    const taskTracking = await (0, TaskTrackingService_1.getTaskTracking)(plugin);
    // Get project information
    const projects = await getProjectInfo(app, settings, tasks);
    // Get file objects for modification dates
    const files = new Map();
    for (const file of app.vault.getMarkdownFiles()) {
        if ((0, areaUtils_1.isInTasksFolder)(file.path, settings) && !(0, areaUtils_1.isTasksFolderFile)(file.path, settings)) {
    ...
    // Identify issues
    const staleTasks = identifyStaleTasks(tasks, taskTracking, settings.healthCheckStaleTaskDays, settings.healthCheckUnmodifiedTaskDays, settings);
    ...
    const tasksNeedingBreakdown = identifyTasksNeedingBreakdown(tasks, settings.healthCheckBreakdownTitleLength, settings.healthCheckBreakdownKeywords);
    const recurringIssues = analyzeRecurringTasks(tasks);
    const cleanupSuggestions = generateCleanupSuggestions(tasks, projects, files, settings);
    return {
        metrics,
        staleTasks,
        quickWins,
 ...
 */
async function loadAllTasks(app, settings) {
    const files = app.vault.getMarkdownFiles()
        .filter(f => (0, areaUtils_1.isInTasksFolder)(f.path, settings) && !(0, areaUtils_1.isTasksFolderFile)(f.path, settings));
    return await (0, taskUtils_1.loadTasksFromFiles)(app, files, settings);
}
/**
 ...
 */
async function getProjectInfo(app, settings, tasks) {
    ...
    for (const task of tasks) {
        ...
        if (existing) {
            ...
            projectMap.set(key, {
                ...
                area: task.area,
                count: 1,
                file: file instanceof obsidian_1.TFile ? file : undefined
            });
        }
    }
 ...
 */
function calculateHealthMetrics(tasks, projects) {
 ...
 */
function identifyStaleTasks(tasks, taskTracking, staleThreshold, unmodifiedThreshold, settings) {
    const stale = [];
    const activeTasks = tasks.filter(t => !t.checked);
    for (const task of activeTasks) {
        // Skip tasks in Someday Maybe folders - these are intentionally deferred
        const isInSomedayMaybe = (0, somedayMaybeUtils_1.isInSomedayMaybeFolder)(task.path, settings, app) ||
            task.area === "Someday Maybe";
        if (isInSomedayMaybe) {
            continue;
        }
        const taskId = (0, TaskTrackingService_1.getTaskId)(task);
        const age = (0, TaskTrackingService_1.calculateTaskAge)(taskTracking, taskId);
        const daysSinceModified = (0, TaskTrackingService_1.calculateDaysSinceModified)(taskTracking, taskId);
        const daysSinceReviewed = (0, TaskTrackingService_1.calculateDaysSinceReviewed)(taskTracking, taskId);
        const reasons = [];
 ...
 */
function identifyQuickWins(tasks, keywords) {
    const quickWins = [];
    const activeTasks = tasks.filter(t => !t.checked);
 ...
 */
function identifyMustMoveItems(tasks, settings) {
    const mustMove = [];
    const activeTasks = tasks.filter(t => !t.checked);
    const today = (0, dateUtils_1.getMomentNow)();
    ...
    for (const task of activeTasks) {
        ...
        if (task.due) {
            const dueDate = (0, dateUtils_1.parseMomentDate)(task.due);
 ...
 */
function identifyOversizedTasks(tasks) {
    const oversized = [];
    const activeTasks = tasks.filter(t => !t.checked);
 ...
 */
function identifyTasksNeedingBreakdown(tasks, titleLengthThreshold, keywords) {
 ...
 */
function analyzeRecurringTasks(tasks) {
    const issues = [];
    const activeTasks = tasks.filter(t => !t.checked);
    // Track recurring tasks by pattern and project
    const recurringByPattern = new Map();
    for (const task of activeTasks) {
        if (!task.recur)
            continue;
        // Check for broken syntax
        if (!(0, Recurrence_1.isValidRecurrencePattern)(task.recur)) {
            issues.push({
                task,
 ...
 */
function generateCleanupSuggestions(tasks, projects, files, settings) {
    const suggestions = [];
    const activeTasks = tasks.filter(t => !t.checked);
