 ...
 */
function filterTasks(tasks, currentTab, filters, app, settings) {
    ...
    if (currentTab === "today-overdue") {
        ...
        if (filters.due === "today") {
            ...
            if (!isNaN(days)) {
                const endDate = (0, dateUtils_1.add)(days, "days", today);
                rows = rows.filter(t => t.due && t.due >= today && t.due <= endDate);
            }
        }
        else if (filters.due === "this-week") {
            const weekStart = (0, dateUtils_1.startOf)("week");
            const weekEnd = (0, dateUtils_1.endOf)("week");
            rows = rows.filter(t => t.due && t.due >= weekStart && t.due <= weekEnd);
        }
        else if (filters.due === "next-week") {
            const nextWeekStart = (0, dateUtils_1.startOf)("week", (0, dateUtils_1.add)(1, "weeks"));
            const nextWeekEnd = (0, dateUtils_1.endOf)("week", (0, dateUtils_1.add)(1, "weeks"));
            rows = rows.filter(t => t.due && t.due >= nextWeekStart && t.due <= nextWeekEnd);
        }
        else if (filters.due === "this-month") {
            const monthStart = (0, dateUtils_1.startOf)("month");
            const monthEnd = (0, dateUtils_1.endOf)("month");
            rows = rows.filter(t => t.due && t.due >= monthStart && t.due <= monthEnd);
        }
        else if (filters.due === "next-month") {
            const nextMonthStart = (0, dateUtils_1.startOf)("month", (0, dateUtils_1.add)(1, "months"));
            const nextMonthEnd = (0, dateUtils_1.endOf)("month", (0, dateUtils_1.add)(1, "months"));
            rows = rows.filter(t => t.due && t.due >= nextMonthStart && t.due <= nextMonthEnd);
        }
    }
    // Apply other filters (common to all tabs)
    if (filters.area !== "All")
        rows = rows.filter(t => (t.area || "") === filters.area);
 ...
 */
function sortTasks(tasks, currentTab, settings) {
    const today = (0, dateUtils_1.formatISODate)(new Date());
    ...
    sorted.sort((a, b) => {
        ...
        if (currentTab === "today-overdue") {
            ...
            const getGroup = (overdue, dueToday, isNowTagOnly) => {
                if (overdue)
                    return 1;
                if (dueToday)
                    return 2;
