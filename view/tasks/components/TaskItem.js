...
exports.renderTaskItem = renderTaskItem;
const PromptModal_1 = require("../../../ui/PromptModal");
const NLDate_1 = require("../../../services/NLDate");
const taskFormatting_1 = require("../utils/taskFormatting");
 ...
 */
function renderTaskItem(host, app, settings, task, callbacks) {
    ...
    const priorityClass = (0, taskFormatting_1.getPriorityColorClass)(task.priority, settings);
    ...
    if (task.recur) {
        ...
        recurIcon.addEventListener("click", async (e) => {
            e.stopPropagation();
            const modal = new PromptModal_1.PromptModal(app, "Set recurrence", task.recur || "", "Examples: 'every Tuesday', 'every 10 days', 'every 2 weeks on Tuesday'");
    ...
    (0, taskFormatting_1.renderDescriptionLine)(title, task.title);
    ...
    if (task.scheduled) {
        ...
        scheduledText.textContent = (0, taskFormatting_1.formatScheduledDate)(task.scheduled);
        ...
        scheduledContainer.addEventListener("click", async () => {
            ...
            const modal = new PromptModal_1.PromptModal(app, "Set scheduled date (today / 2025-11-10)", defaultValue, undefined, "scheduled", task.title);
    ...
    if (task.due) {
        ...
        dueText.textContent = (0, taskFormatting_1.formatDueDate)(task.due);
        // Apply red styling if overdue
        if ((0, taskFormatting_1.isOverdue)(task.due)) {
        ...
        dueContainer.addEventListener("click", async () => {
            ...
            const modal = new PromptModal_1.PromptModal(app, "Set due date (today / 2025-11-10)", defaultValue, undefined, "due", task.title);
            const next = await modal.prompt();
            if (next == null)
                return; // User cancelled
            if (next.trim() === "") {
    ...
    const priorityColorClass = (0, taskFormatting_1.getPriorityColorClass)(task.priority, settings);
    ...
    const allLabels = (0, taskFormatting_1.extractLabels)(task);
    ...
    if (task.description) {
        ...
        descLines.forEach((line, idx) => {
            if (line.trim().length > 0) {
                ...
                (0, taskFormatting_1.renderDescriptionLine)(lineEl, line);
            }
        ...
        const descIcon = leftSide.querySelector(".task-description-icon");
        if (descIcon) {
 ...
 */
function renderInlineValidationFeedback(container, results) {
 ...
 */
function startEditingTitle(titleEl, task, callbacks) {
    ...
    // Initial validation
    if (parentContainer) {
        const results = (0, ValidationService_1.validateTaskTitle)(currentText);
        renderInlineValidationFeedback(parentContainer, results);
    }
    ...
    const finishEditing = async () => {
        ...
        if (newTitle && newTitle !== currentText) {
            ...
            (0, taskFormatting_1.renderDescriptionLine)(newTitleEl, currentText);
