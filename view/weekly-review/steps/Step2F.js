...
exports.renderStep2F = renderStep2F;
 ...
 */
async function renderSomedayMaybeProjectCard(host, app, settings, project, callbacks) {
    ...
    (0, obsidian_1.setIcon)(activateBtn, "activity");
 ...
 */
async function renderStep2F(host, app, settings, wizardState, callbacks) {
    host.empty();
    // Guidance text
    ...
    const somedayMaybeProjects = await (0, WeeklyReviewService_1.fetchSomedayMaybeProjects)(app, settings);
    ...
    const unreviewedProjects = somedayMaybeProjects.filter(p => !wizardState.reviewedSomedayMaybeProjects.has(p.path));
    const reviewedProjects = somedayMaybeProjects.filter(p => wizardState.reviewedSomedayMaybeProjects.has(p.path));
