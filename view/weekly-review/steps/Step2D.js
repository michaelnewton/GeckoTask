...
exports.renderStep2D = renderStep2D;
 ...
 */
async function renderStep2D(host, app, settings, wizardState, callbacks) {
    host.empty();
    // Guidance text
    ...
    const projects = await (0, WeeklyReviewService_1.fetchProjectsWithTasks)(app, settings);
    ...
    const unreviewedProjects = projects.filter(p => !wizardState.reviewedProjects.has(p.path));
    const reviewedProjects = projects.filter(p => wizardState.reviewedProjects.has(p.path));
    ...
    if (unreviewedProjects.length > 0) {
        ...
        for (const project of unreviewedProjects) {
            await (0, ProjectCard_1.renderProjectCard)(projectsList, app, settings, project, {
                ...callbacks,
                onAddTask: callbacks.onAddTask,
                onMarkReviewed: async (projectPath) => {
    ...
    if (wizardState.showReviewedProjects && reviewedProjects.length > 0) {
        ...
        for (const project of reviewedProjects) {
            await (0, ProjectCard_1.renderProjectCard)(reviewedList, app, settings, project, {
                ...callbacks,
                onAddTask: callbacks.onAddTask,
